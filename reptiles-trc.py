...
for file in files:
    corte = file.split('/')
    animal = corte[-2]
    especie = file.split(corte[-1])[0] + 'trc'
    # especie ='igls_tcrs'
    if os.path.exists(especie) is False:
        os.mkdir(especie)
    print(animal, file)
    qquery = '/Users/franciscogambondeza/.mounty/rap/TCRs reptiles/training/query-trc'
    ...
    chs = ['back', 'trac', 'trbc', 'trgc', 'trdc', 'trec','trtc']

    path = '/Users/franciscogambondeza/.mounty/rap/TCRs reptiles/training'
    ...
    ccc = 0
    o = open(especie + '/result-trc.fasta', 'w')
    ...
    for rio in SeqIO.parse(bas, 'fasta'):
        ...
        for a, b, c, d in elegidos:
            if int(a) == int(par[2]) and int(b) == int(par[3]) and float(d) >= 0.2:
